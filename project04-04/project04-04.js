...
// Event handlers to run when the cash or bill value changes
cashBox.addEventListener("change", runTheRegister);
billBox.addEventListener("change", runTheRegister);

// Function to reset values on the webpage
...
function runTheRegister() {
   ...
   try {
      // Check if changeValue is not greater than or equal to zero
      if (changeValue < 0) {
         throw new Error("Cash amount doesn’t cover the bill");
      }

      // Update the changeBox value with formatted currency and calculate change
      changeBox.value = formatCurrency(changeValue);
      calcChange(changeValue);
   } catch (error) {
      // Catch and handle the error by displaying a warning message
